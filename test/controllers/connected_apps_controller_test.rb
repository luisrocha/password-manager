...
class ConnectedAppsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'creates a mobile pairing code when vault is unlocked' do
    ...
    assert_match(/\A[ABCDEFGHJKMNPQRSTUVWXYZ23456789]{4}-[ABCDEFGHJKMNPQRSTUVWXYZ23456789]{4}\z/,
                 response_body.fetch('code'))
    assert_equal 300, response_body.fetch('expires_in_seconds')
    assert_equal compact_mobile_vault_payload,
                 Rails.cache.read(MobileVaultPairing.cache_key(response_body.fetch('code')))
  end
